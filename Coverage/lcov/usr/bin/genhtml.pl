...
#   geninfo script. Call it with --help and refer to the genhtml1.pl man page
#   to get information on usage and available options.
#
##
# 此脚本根据geninfo脚本创建的.info文件生成HTML输出
##
#
# History:
...
#                the directory name.  I found that genhtml1.pl.pl died when it
...
## 单击函数名应该会在函数第一行代码前几行的位置显示源代码。此数字指定行中的偏移量。
our $func_offset = 2;
...
## 数据相关原型 sub 表示子程序 与方法类似 可以出现在程序的任何地方 以下是子程序的前向引用
sub print_usage(*);
sub gen_html();
sub html_create($$);
sub process_dir($);
...
Create HTML output for coverage data found in INFOFILE. Note that INFOFILE
may also be a list of filenames.
为INFOFILE中的覆盖数据创建HTML输出。请注意，INFOFILE 也可以是一个文件名列表。

Misc:
  -h, --help                        打印此帮助，然后退出
  -v, --version                     打印版本号，然后退出
  -q, --quiet                       不打印进度消息
      --config-file FILENAME        指定配置文件位置
      --rc SETTING=VALUE            覆盖配置文件设置
      --ignore-errors ERRORS        出错后继续（源）

Operation:
  -o, --output-directory OUTDIR     将HTML输出写入OUTDIR
  -s, --show-details                生成详细目录视图
  -d, --description-file DESCFILE   从DESCFILE读取测试用例描述
  -k, --keep-descriptions           不要删除未使用的测试说明
  -b, --baseline-file BASEFILE      使用BASEFILE作为基线文件
  -p, --prefix PREFIX               从所有目录名中删除前缀
      --no-prefix                   不要从目录名中删除前缀
      --(no-)function-coverage      启用（禁用）功能覆盖显示
      --(no-)branch-coverage        启用（禁用）分支覆盖范围显示

HTML output:
  -f, --frames                      将HTML框架用于源代码视图
  -t, --title TITLE                 在所有页面的标题中显示标题
  -c, --css-file CSSFILE            使用外部样式表文件CSSFILE
      --no-source                   不创建源代码视图
      --num-spaces NUM              在源代码视图中将制表符替换为NUM spaces
      --highlight                   突出显示仅包含转换数据的行
      --legend                      在HTML输出中包含颜色图例
      --html-prolog FILE            将文件用作生成页面的HTML序言
      --html-epilog FILE            将文件用作生成的页面的HTML结束语
      --html-extension EXT          使用EXT作为页面的文件扩展名
      --html-gzip                   使用gzip压缩HTML
      --(no-)sort                   启用（禁用）排序覆盖率视图
      --demangle-cpp                删除C++函数名
      --precision NUM               设置覆盖率精度
      --missed                      将未命中计数显示为负数
...
# file list.
## 返回指定的 found 和 hit 值的相对值，该值用于对文件列表中的相应项进行排序。
...
# found/hit data.
## 返回一个字符串，包含指定的 found 和 hit 数据的整体信息。
...
#
##  根据CONV: OLD_NAME -> NEW_NAME重命名INFO中的所有函数名。
#如果两个函数的名字相同，假设它们是同一个源函数的不同对象代码实现
...
# Die on error.
#
# 从.info文件INFO_FILENAME的内容生成一组HTML页面。文件将被写入当前目录。
# 如果提供，测试用例说明将从.test文件TEST_FILENAME中读取并包含在输出中。
#
# 报错后结束。
...
## 读入info_FILENAME指定的.info文件的内容。数据将作为对包含以下映射的哈希的引用返回：

# %result: for each filename found in file -> \%data
## %result: 对于文件->\%data中找到的每个文件名
...
# %testdiffdata:  name of test affecting this file -> \%testdiffcount
##
# %testdata   : 影响此文件的测试的名称 -> \%testcount
# %testfncdata: 影响此文件的测试的名称 -> \%testfnccount
# %testbrdata:  影响此文件的测试的名称 -> \%testbrcount
# %testdiffdata:  影响此文件的测试的名称 -> \%testdiffcount


# %testcount   : line number   -> execution count for a single test # 行号->单个测试的执行计数
# %testfnccount: function name -> execution count for a single test # 函数名->单个测试的执行计数
# %testbrcount : line number   -> branch coverage data for a single test # 行号->单个测试的分支覆盖数据
# %sumcount    : line number   -> execution count for all tests # 行号->所有测试的执行计数
# %sumfnccount : function name -> execution count for all tests # 函数名->所有测试的执行计数
# %sumbrcount  : line number   -> branch coverage data for all tests # 行号->所有测试的分支覆盖数据
# %funcdata    : function name -> line number # 函数名->行号
# %checkdata   : line number   -> checksum of source code line # 行号->源代码行校验和
# $brdata      : vector of items: block, branch, taken # 项目向量：块、分支、取数
...
#
## 请注意，引用相同文件和测试名称的.info文件部分将通过添加所有执行计数自动组合。
#
#
...
# this file.
#
## 注意，如果INFO_FILENAME以“.gz”结尾，则假定该文件是使用GZIP压缩的。如果可用，将使用GUNZIP解压缩此文件。
#
# Die on error.
#
...
## 从read_info_file()生成的结构条目中获取数据。返回一个哈希值的引用列表。
...
#
# DATA1_REF and DATA2_REF are references to hashes containing a mapping （DATA1_REF和DATA2_REF是对包含映射的哈希的引用。）
#
#   line number -> execution count （行数->执行次数）
#
# Return a list (RESULT_REF, LINES_FOUND, LINES_HIT) where RESULT_REF
# is a reference to a hash containing the combined mapping in which
# execution counts are added.
#
#返回一个列表(RESULT_REF, LINES_FOUND, LINES_HIT)，其中RESULT_REF是对包含组合映射的哈希的引用，在这个映射中添加了执行次数。
...
# but does not match.
## 合并REF1和REF2中定义的校验和列表，并返回对结果哈希的引用。如果一行的校验和在两个哈希中定义但不匹配，则结束。
...
# added_testfncdata.
## 为多个测试添加函数调用计数数据。返回引用 added_testfncdata.
...
## 将分支覆盖率数据转换回brcount格式。如果指定brcount，则转换后的数据直接插入brcount。
...
#
# 如果add是brunadd，则添加分支覆盖率数据并添加返回列表brcount。
# 如果add为BR_SUB，则从brcount1减去brcount2的值，然后返回brcount_SUB。
# 如果设置了inplace，则将结果插入brcount1。
...
# added_testbrdata.
## 为多个测试添加分支覆盖率数据。返回对添加的_testbrdata的引用。
...
## 将条目_REF1和条目_REF2引用的.info数据条目散列合并。返回对结果哈希的引用。
...
## 在info_REF1和info_REF2引用的哈希中合并.info数据。返回对结果哈希的引用。
...
#
##  在FILENAME_LIST中搜索一个目录前缀，该前缀对尽可能多的列表条目是通用的，这样删除这个前缀将使所有缩短文件名的长度之和最小化，同时观察到没有文件名的父目录少于MIN_DIR。
...
# in SUBDIRECTORY.
## 返回在子目录中应用时引用基路径的相对路径字符串。

# 例如: get_relative_base_path("fs/mm") -> "../../"
#
...
## 读入文件TEST_FILENAME，其中包含以下格式的测试说明：
...
## 创建子目录DIR_NAME（如果它不存在），包括它的所有父目录。
...
## 编写包含所有测试用例描述的HTML文件。描述是对包含映射的哈希的引用

#   test case name -> test case description   ## 测试用例名称->测试用例描述
#
# Die on error.
## 报错后结束
...
## 在当前目录中为HTML输出创建所有必需的.png文件。png文件用作条形图。
...
## 编写级联样式表文件gcov.css公司到当前目录。此文件定义所有生成的HTML页面的基本布局属性。
...
### 返回一个包含HTML代码的字符串，该代码实现了覆盖率为cover_hit*100/cover_found的条形图显示。
...
## 将HTML_代码写出到FILEHANDLE，同时删除每行HTML_代码中的前导制表符标记。
...
#
## 将所有HTML文件通用的HTML prolog写入FILEHANDLE。 PAGETITLE将用作HTML页面标题。BASE_DIR包含指向基目录的相对路径。
...
## 返回一个HTML行列表，这些行表示源代码视图中指定的分支覆盖率数据。
...
# by gen_png()
## 编写格式化的源代码行。按照gen_png（）需要的格式返回一行
...
# break this frameset.
## 将HTML页页脚写入FILEHANDLE。当此页面嵌入框架集中时，应设置BREAK_FRAMES，单击URL链接将断开此框架集。
...
## 写一个完整的标准页眉。类型可以是（0，1，2，3，4）对应（目录视图头，文件视图头，源代码视图头，测试用例描述头，函数视图头）
...
# stats: [ lines_found, lines_hit, fn_found, fn_hit, br_found, br_hit, diff_found, diff_hit,
#          link_name, line_rate, fn_rate, br_rate, diff_rate ]
#
...
# otherwise.
## 如果FILEVIEW为true，则第一列的标题为“Filename”，否则为“Directory name”。
...
# a list (found, hit)
## 以列表（found，hit）的形式返回散列（linenumber->execution count）中的条目（found）和执行计数大于零（hit）的条目的计数
...
#
# get_diff_found_and_hit(sumdiffccount)
#
# Return (diff_found, diff_hit) for sumdiffccount
#
...
# Die on error.
#
# 写一个源代码文件的HTML视图。返回一个包含gen_png()所需数据的列表。
#
# 错误则结束。
...
## 根据sort_type的值，返回按名称（类型0）或相关调用计数（类型1）排序的函数列表。
...
## 编写一个HTML表，列出源文件中的所有函数，还包括函数调用计数和每个函数内部的行覆盖率。
#
#  Die on error.
## 报错后结束
...
# is not set.
## 只有当$quiet标志没有被设置时，才使用printf将PRINTF_PARAMETER写入stdout。
...
## 从data_REF中的实际数据中减去baseline_REF引用的基线哈希中找到的执行计数。
...
## 从全局哈希%test_description中删除%info_data中不存在的所有测试描述。
...
## 如果FILENAME以PREFIX from PREFIXES开头，则从FILENAME中删除PREFIX并返回结果字符串，否则返回FILENAME。
...
## 读取配置文件文件名，并返回一个包含找到的所有有效 key=value 对的哈希的引用。
...
## 如果定义了文件名，则返回文件的内容。否则返回默认的HTML prolog。 报错后结束
...
## 如果定义了文件名，则返回文件的内容。否则返回默认的HTML epilog。 报错后结束
...
# characters wide.
#
# 返回命中和找到值的覆盖率[0..100]。只有命中为0时才会返回0。
# 只有在找到HIT等于时才返回100。精度指定结果的精度。
# 后缀定义一个字符串，如果找到的是非零，则附加到结果中。
# 空格将添加到结果字符串的开头，直到其宽度至少为字符宽度。Line data
